...
if EnableLuaProfiler then
	...
	function(co, ...)
		...
		KLEIProfiler.co_pause( cco )
		KLEIProfiler.co_resume( co )
		local rval = { resume(co, ...) }
		KLEIProfiler.co_pause( co )
		if coroutine.status(co) == 'dead' then
			KLEIProfiler.co_destroy( co )
		end
		KLEIProfiler.co_resume( cco )
		return unpack(rval)
	end
...
MOAIHttpTask = MOAIHttpTaskCurl or MOAIHttpNaCl or false
KLEIMetric = KLEIMetric or false
...
KLEISteamWorkshop = KLEISteamWorkshop or false
KLEIAchievements = KLEIAchievements or false
